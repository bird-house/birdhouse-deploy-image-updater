...
if grep -q 'export FINCH_IMAGE="pavics/weaver:1.13.2-worker"' "last-diff-result.log"; then
    echo "[INFO] [bump_weaver-worker_to_1.13.2-worker] The commit content looks good"
    # echo
    # cat last-diff-result.log
else
    echo "ERROR - wrong commit content. Exiting."
    exit 1
fi
...
if grep -q 'export FINCH_IMAGE="birdhouse/finch:version-0.5.4"' "last-diff-result.log"; then
    echo "[INFO] [bump_finch_to_version-0.5.4] The commit content looks good"
    # echo
    # cat last-diff-result.log
else
    echo "ERROR - wrong commit content. Exiting."
...
if grep -q 'export FINCH_IMAGE="pavics/weaver:1.13.3-worker"' "last-diff-result.log"; then
    echo "[INFO] [bump_weaver-worker_to_1.13.3-worker] The commit content looks good"
    # echo
    # cat last-diff-result.log
else
    echo "ERROR - wrong commit content. Exiting."
